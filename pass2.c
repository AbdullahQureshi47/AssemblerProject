...
void pass2 (FILE * fp, LabelTable table) {
 ...
 char* getBin(int dec, int range)
 {
   ...
   while(k <= range)
   {
     ...
     {
       (void)strcat(binary[k], "1");
       dec -= pow(2,(range-k));
     }
     //a zero is placed on the binary string
     else
       (void)strcat(binary[k], "0");
     k++;
   }
 ...
 /*Takes the instruction and searches for corresponding binary from file.
  *Constructed in a similar fashion to pass1 that alyce wrote.
 ...
 /* returns a copy of the label table that was constructed */
 {
     ...
     for (PC = 0; fgets (inst, BUFSIZ, fp); PC += 4)
     {
         ...
         *tokEnd = '\0';      /* truncate everything after first instruction */
         ...
         if(info[7]==(char*)'0')
         {
             getRegs(&rs, &rt, &rd, NULL, NULL, tokBegin, tokEnd);
             shamt = "0";
         ...
         {
             getRegs(NULL,&rt, &rd, &shamt, NULL, tokBegin, tokEnd);
             rs = "$zero";
         }
         //gets reister rd for jr operation
         else if(info[7]==(char*)'2')
         {
             getRegs(&rs,NULL,NULL,NULL,NULL, tokBegin, tokEnd);
         ...
         {
             getRegs(&rs, NULL, &rd, &shamt, NULL, tokBegin, tokEnd);
         }
         //gets registers rd, shamt, rs lw and sw operation
         else if(info[7]==(char*)'4')
         {
             getRegs(&shamt, NULL, &rd, &rs, NULL, tokBegin, tokEnd);
         }
         //gets registers rd, shamt for lui operation
         else if(info[7]==(char*)'5')
         {
             getRegs(NULL, NULL, &rd, &shamt, NULL, tokBegin, tokEnd);
             rs="$zero";
         ...
         if(strcmp(info[6],"I")==SAME)
         {
             ...
             if(info[7] == (char*)'6')
                 (void)fprintf(stdout, "%s", getBin(address, 15));
             else
                 (void)fprintf(stdout, "%s", getBin(shamtInt, 15));
             (void)fprintf(stdout,"\n");
         ...
         if(strcmp(info[6],"R")==SAME)
         {
             ...
             (void)fprintf(stdout, "%s", getRegisterCode(rd));
             (void)fprintf(stdout, "%s", getBin(shamtInt, 4));
         ...
         {
             (void)fprintf(stdout, "%s", info[0]);
             (void)fprintf(stdout, "%s", getBin(address, 25));
             (void)fprintf(stdout,"\n");
